...
> `geny-executor` Pipeline 기반 다중 세션 에이전트 관리 시스템

## 프로젝트 개요

Geny Agent는 [`geny-executor`](https://github.com/CocoRoF/geny-executor) Pipeline 엔진 위에서 **다중 에이전트 세션**을 관리하는 FastAPI 백엔드입니다. 세션 간 브로드캐스트·DM 채팅, MCP 도구 통합, 벡터 메모리, SSE/WebSocket 실시간 스트리밍을 지원합니다.

### 핵심 기능

- **다중 세션** — 독립적인 에이전트 세션을 병렬로 실행·관리 (각 세션이 `geny-executor` Pipeline 을 래핑)
- **Pipeline 실행** — `geny-executor` 의 Stage 기반 실행 (input → tool dispatch → API → parse → yield)
