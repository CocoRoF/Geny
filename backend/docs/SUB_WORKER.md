...
The Sub-Worker binding makes the two-agent split **first-class**:

- The VTuber's creation request carries the Worker's config (`sub_worker_model`, `sub_worker_system_prompt`, `sub_worker_env_id`).
- The system spawns the Worker atomically as part of VTuber startup.
- The Worker's `session_id` is injected into the VTuber's system prompt as a `## Sub-Worker Agent` block, so the VTuber knows who to talk to for complex work.
- The Worker's `linked_session_id` back-references the VTuber, so when the Worker finishes a task it knows who to reply to.

Delegation is just `geny_send_direct_message` from the VTuber to the Sub-Worker's `session_id`. The reply arrives in the VTuber's inbox tagged `[SUB_WORKER_RESULT]` and is summarized in persona.
...
A VTuber session has **exactly one** Sub-Worker. A Worker session is bound to **at most one** VTuber.

- Enforced at creation: the auto-pair block in `AgentSessionManager.create_session` runs exactly once per VTuber request, guarded by `session_type != "sub" and not linked_session_id`.
- Not enforced at runtime: nothing stops a caller from manually calling `create_session(role=WORKER, linked_session_id=<vtuber_id>, session_type="sub")` a second time. Don't do that. See **Non-goals**.
...
- **VTuber terminates**: the Sub-Worker is a leaf and can be cleaned up by the session store's normal termination path. `agent_session_manager.stop_session(vtuber_id)` does **not** currently cascade to the Sub-Worker — the Worker survives and can be targeted directly. This is intentional (see the **Orphan Worker** failure mode below).
- **Worker terminates**: the VTuber's `## Sub-Worker Agent` block now references a dead session. Subsequent `geny_send_direct_message` calls will fail at the tool layer with `is_error=True`. The VTuber remains functional but delegation is broken until a new VTuber session is created.

### I4. No hot-swap

The Sub-Worker's `session_id`, `env_id`, `model`, and `system_prompt` are frozen at VTuber creation time. There is no API to swap one Worker out for another while keeping the VTuber alive.

- To change Worker config, create a new VTuber session with the desired `sub_worker_*` fields.

### I5. Sub-Workers cannot themselves bind

A Worker session with `session_type == "sub"` cannot trigger another auto-pair spawn. The recursion guard in `AgentSessionManager.create_session` is:
...
The `session_type != "sub"` clause is the load-bearing check. `not linked_session_id` is a second predicate that catches any request already carrying a link (belt-and-braces).
...
AgentSessionManager.create_agent_session(role=VTUBER, sub_worker_env_id=...)
    ...
    │
    ├─ recursion guard: role==VTUBER, session_type!="sub", linked_session_id is None  ✓
    │
    ├─ build worker_request:
    │     role                      = WORKER
    │     env_id                    = request.sub_worker_env_id   (may be None)
    │     model                     = request.sub_worker_model    (may be None)
    ...
    │     └─ guard: session_type=="sub"   ✗  (no further spawn)
    │     └─ returns W1
    │
    ├─ inject V1._system_prompt += "## Sub-Worker Agent\n… session_id=`W1` …"
    ├─ log "🔗 Sub-Worker created: W1"
    │
    ▼
...
| Initiator | Effect on VTuber | Effect on Sub-Worker |
|-----------|------------------|------------------------|
...
| Idle monitor | V1 stays RUNNING (VTubers are always-on) | W1 stays RUNNING (Sub-Workers share the always-on policy via `AgentSession._is_always_on`) |
...
When the backend restarts, `AgentSessionManager._build_system_prompt` re-runs during session restoration. For VTubers, the `role == "vtuber" and request.linked_session_id` branch re-injects the `## Sub-Worker Agent` block using the persisted `linked_session_id`. The wording matches the auto-pair creation path verbatim, so a VTuber reads the same system prompt across restarts.

---

## env_id resolution

Sub-Workers flow through `resolve_env_id(role, explicit)` just like any other session (see `plan/02_default_env_ids.md`):

| `sub_worker_env_id` passed | Effect |
|------------------------------|--------|
...
| `"template-developer-env"` | Sub-Worker runs with the developer manifest (broader tool surface, dev-oriented system prompt) |
| Unknown env_id | `resolve_env_id` raises at VTuber creation time → client gets an error, no partial state |

Under plan/02, envs own stage layout and `manifest.tools.built_in` / `.external` carry tool selection. The Sub-Worker has no `workflow_id` / `graph_name` / `tool_preset_id` overrides in the auto-pair path — if you need something custom, author a new env.
...
Handling:
- `geny_send_direct_message` is fire-and-forget — it only confirms the DM was queued, not that the Worker has processed it. The normal path is: VTuber calls send → returns quickly → Worker processes asynchronously → Worker replies with its own `geny_send_direct_message` tagged `[SUB_WORKER_RESULT]` → arrives in VTuber's inbox → VTuber's next agent turn drains inbox.
- If the VTuber wants to wait on a specific result, it should stay in persona ("잠시만, 알아볼게!") and let the `[SUB_WORKER_RESULT]` trigger wake it up when the Worker finishes.

### Orphan Worker (VTuber gone, Worker alive)

Symptom: VTuber session stopped or crashed; Sub-Worker still in the session store.
...
- The session store's bulk cleanup (`DELETE FROM sessions WHERE ended_at < NOW() - INTERVAL '7 days'`) eventually removes it. There is no shorter TTL specific to orphaned Sub-Workers.

### Recursion guard failure

Symptom: A VTuber request slips past the recursion guard and spawns an infinite chain of Sub-Workers.

Handling:
- The guard is `session_type != "sub" and not linked_session_id`. A bug that sets neither field on a Sub-Worker request would break this. Smoke tests in PRs 18 + 20 verify the current guard holds.
- If it ever does fail in practice, `SessionStore.count_active_sessions()` will spike and the idle monitor log will show many `🔗 Sub-Worker created:` lines in a tight window. Kill the runaway sessions manually.

---

## Non-goals

These are **explicitly out of scope** for the Sub-Worker contract as designed. They are not bugs; they are features we chose not to build.

### Multi-Worker fan-out

*"A VTuber with two Sub-Workers — one for code, one for research."*
...
*"Replace the Sub-Worker without recreating the VTuber."*
...
*"Two VTubers sharing one Sub-Worker to save resources."*

Not supported. The Worker's `linked_session_id` is a single field, not a list. Back-replies via `[SUB_WORKER_RESULT]` target exactly one VTuber. Sharing would require a routing layer.

### Dynamic rebind

*"Re-bind the Worker to a different VTuber when the first one ends."*

Not supported. When a VTuber terminates, its Sub-Worker becomes an orphan Worker (see **Failure modes**), not a pool resource. If a future need emerges, it would be a new feature — not a repair of the current contract.
...
|------|---------------------|
| `service/sessions/models.py` | `CreateSessionRequest.sub_worker_model` / `_system_prompt` / `_env_id`; `session_type` canonical values include `"sub"` (with legacy `"bound"` / `"cli"` normalized on read) |
...
| `controller/tts_controller.py` | Strips `[SUB_WORKER_RESULT]` / `[CLI_RESULT]` (legacy) / `[ACTIVITY_TRIGGER]` tag literals before TTS output |
...
- [`EXECUTION.md`](EXECUTION.md) — agent executor, `[SUB_WORKER_RESULT]` emission, tool call flow
