...
> 작성일: 2025-07-14
> 목표: VTuber / Sub-Worker / 일반 에이전트의 시스템 프롬프트를 깔끔하고 목적에 맞게 재설계
...
AgentSessionManager._build_system_prompt()
  └─ build_agent_prompt()
       ...
       └─ + shared_folder_path
  └─ + memory_context
  └─ + VTuber linked Sub-Worker 정보 (vtuber only)
...
| **P7** | Sub-Worker의 프롬프트가 VTuber와 별도로 동작하지만, VTuber 연동 컨텍스트가 부족 | Sub-Worker가 "누구에게 보고해야 하는지" 정도만 알고, VTuber의 사용자가 어떤 맥락을 가졌는지 모름 |
...
- "복잡한 작업은 Sub-Worker에게 위임하라" (VTuber에서)
...
- Sub-Worker: 작업자이므로 상세 가능하지만 **불필요한 반복 제거**
...
```python
ROLE_IDENTITY = {
    "worker":     "You are a Geny Sub-Worker agent.",
...
현재 `vtuber.md` 문제:
- "Direct Response" / "Delegate to Sub-Worker" 구분이 너무 상세하고 rigid
...
- Delegate coding, file operations, complex research, and multi-step technical tasks
  to your paired Sub-Worker via `geny_send_direct_message`
...
- [SUB_WORKER_RESULT]: Summarize the Sub-Worker's work result conversationally with appropriate emotion
```
...
### Phase 3: Sub-Worker 프롬프트 개선

#### 3.3.1 `prompts/templates/sub-worker-default.md` 개선
...
```python
"Your paired Sub-Worker session ID: `{linked_session_id}`"
```

**개선**: Sub-Worker 쪽에도 VTuber session ID와 사용자 맥락 전달
```python
# VTuber → Sub-Worker 방향
f"## Paired Sub-Worker Agent\n"
f"Session ID: `{linked_session_id}`\n"
f"Send tasks via `geny_send_direct_message`. Results will come back to your inbox."

# Sub-Worker → VTuber 방향 (현재는 없음 - 추가 필요)
...
| 6 | Sub-Worker 프롬프트 개선 | ⭐ | 중간 |
| 7 | Sub-Worker ↔ VTuber 양방향 세션 연동 | ⭐⭐ | 중간 |
...
## Linked Sub-Worker Agent
Your paired Sub-Worker session ID: `def456`
...
...
- Delegate coding, file ops, complex research to Sub-Worker via `geny_send_direct_message`
- Acknowledge → send → inform user → summarize result

## Triggers
- [THINKING_TRIGGER]: Reflect, check tasks, optionally initiate
- [SUB_WORKER_RESULT]: Summarize work result with appropriate emotion
...
### Q5: Sub-Worker 에게 VTuber session_id를 전달할까?

- 현재 Sub-Worker는 VTuber를 모름 (DM을 받아서 수행하지만, 보고 대상을 프롬프트로는 모름)
- **제안**: 전달 추가 — Sub-Worker가 자율적으로 VTuber에게 보고할 수 있게
...
Phase 3: Sub-Worker 프롬프트 개선
  ├─ prompts/templates/sub-worker-*.md 업데이트
  └─ _build_system_prompt()에 Sub-Worker→VTuber 세션 연동 추가
...
| `backend/prompts/templates/sub-worker-default.md` | VTuber 연동 중심으로 개선 |
| `backend/prompts/templates/sub-worker-detailed.md` | VTuber 연동 중심으로 개선 |
