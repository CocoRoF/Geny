...
const ko: Translations = {
  ...
  settings: {
    ...
    models: {
      ...
      addKey: '계정 추가',
      accountCount: '{n}개',
      orderHint: '위에 있는 계정이 먼저 답합니다. 그 계정이 답할 수 없을 때만 다음으로 넘어갑니다.',
      orderEmpty: '켜 둔 계정이 없습니다 — 아래에서 하나 추가하세요.',
      baseUrl: '주소',
      secret: 'API 키',
  ...
  voiceStudio: {
    ...
    harness: {
      ...
      section: {
        budgets: '한도',
        budgetsHint: '이 에이전트가 한 턴에 얼마나 멀리 갈 수 있는지를 정합니다.',
        basic: '자주 바꾸는 것',
        basicHint: '에이전트가 어떻게 행동할지를 실제로 바꾸는 여덟 가지입니다.',
        advanced: '고급',
        advancedHint: '파이프라인 단계를 아는 상태에서 바꾸세요. 잘못 고르면 턴이 실패할 수 있습니다.',
        locked: '바꿀 수 없는 것',
        lockedHint: 'Geny 가 무엇인지를 정하는 부분입니다. 숨기지 않고 보여 주되, 설정으로는 열려 있지 않습니다.',
      },
      source: {
        default: '기본값',
        manifest: '파이프라인',
      ...
      budget: {
        maxIterations: '한 턴의 최대 반복 횟수',
        maxIterationsHint: '도구를 쓰고 결과를 읽는 왕복을 몇 번까지 할지. 여기 적은 수가 실제로 적용되는 수입니다.',
        contextWindow: '컨텍스트 창',
        contextWindowHint: '대화가 이만큼 차면 앞부분을 요약해 줄입니다. 비워 두면 라우트에서 가장 작은 모델에 맞춥니다.',
        contextWindowPerHop: '라우트의 각 모델: {hops}. 가장 작은 값이 기준입니다.',
        costCeiling: '한 턴의 비용 상한',
        costCeilingHint: '0 이면 상한 없음. 백엔드가 알려 주는 비용만 셀 수 있어서, 비용을 보고하지 않는 구독 계정은 여기에 걸리지 않습니다.',
      },
      question: {
      ...
      locked: {
        ...
        routeDecides: '어떤 모델이 답할지는 라우트가 정합니다. 여기서 또 정하면 답이 두 군데가 됩니다.',
        onlyOption: '선택지가 하나뿐입니다.',
      },
      installed: {
        persistingCompactor: '요약할 때마다 그 요약을 기억에도 남깁니다.',
        memoryAware: '이 에이전트의 기억을 턴의 맥락으로 끌어옵니다.',
      ...
      stage: {
        input: '입력', context: '맥락', system: '시스템', guard: '사전 검사', cache: '캐시',
        api: '호출', token: '토큰', think: '사고', parse: '해석', tool: '도구',
        tool_review: '도구 검토', agent: '위임', task_registry: '작업 원장',
        evaluate: '평가', hitl: '사람 승인', loop: '반복', emit: '출력',
        memory: '기억', summarize: '요약', persist: '보존', yield: '반환',
      },
    },
    settings: {
      engines: {
