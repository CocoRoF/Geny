...
/**
 * Mutation audit log viewer (G11.3).
 *
 * Lists every ``mutation.applied`` event the executor's PipelineMutator
 * emits — strategy swaps, chain reorders, tool bindings, model
 * overrides, etc. Scrollable chronological feed with kind / before /
 * after / actor.
 *
 * Like the other G11 components, derives from the session's logEntries
 * array — no extra subscription.
 */
...
function extractMutations(entries: LogEntry[]): MutationRow[] {
  const out: MutationRow[] = [];
  ...
  return (
    <ul className="px-3 py-2 flex flex-col gap-1 max-h-[280px] overflow-auto">
      {mutations.map((row, idx) => (
        <li
          key={`${row.ts}-${idx}`}
          className="flex items-start gap-2 px-2 py-1.5 rounded-md border border-[var(--border-color)] bg-[var(--bg-primary)] text-[0.6875rem]"
        >
